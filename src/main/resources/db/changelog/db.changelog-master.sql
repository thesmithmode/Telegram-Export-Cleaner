...
CREATE INDEX idx_subscriptions_confirm
    ON chat_subscriptions (confirm_sent_at)
    WHERE confirm_sent_at IS NOT NULL;

-- =============================================================================
-- 006: Поле subscription_id в export_events для аудита итераций подписки.
-- NULL означает ручной экспорт (вызван пользователем напрямую через бот).
-- Не-NULL ссылается на chat_subscriptions.id и позволяет отследить, в рамках
-- какой подписки был сгенерирован данный экспорт (история итераций, дебаг,
-- агрегированная статистика по подписке в дашборде).
-- FK не добавляется через ALTER TABLE в SQLite — целостность обеспечивается
-- на уровне сервиса; индекс покрывает фильтрацию по subscription_id.
-- =============================================================================
