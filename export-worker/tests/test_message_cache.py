...
class TestPageSizeMigration:
    ...
    @pytest.mark.asyncio
    async def test_evict_cache_range_invalidation_without_redis_is_noop(self, tmp_path):
        c = MessageCache(db_path=str(tmp_path / "ranges_no_redis.db"))
        await c.initialize()
        try:
            await c._invalidate_cache_ranges_in_redis(-100123)
        finally:
            await c.close()

    @pytest.mark.asyncio
    async def test_evict_cache_range_invalidation_swallows_redis_errors(self, tmp_path):
        from unittest.mock import AsyncMock
        c = MessageCache(db_path=str(tmp_path / "ranges_redis_error.db"))
        await c.initialize()
        try:
            redis_mock = AsyncMock()
            redis_mock.get = AsyncMock(side_effect=ConnectionError("Redis down"))
            redis_mock.delete = AsyncMock(side_effect=ConnectionError("Redis down"))
            c.redis_client = redis_mock

            await c._invalidate_cache_ranges_in_redis(-100123)

            redis_mock.delete.assert_awaited_once_with("cache:ranges:-100123")
        finally:
            await c.close()
