...
class TestProcessJobErrorMapping:
    ...
    @pytest.mark.asyncio
    async def test_session_invalid_does_not_recover_from_redis_vault(self, worker):
        """SESSION_INVALID terminates without reading Redis session vault bearer credentials."""
        from unittest.mock import patch, AsyncMock as _AM
        ...
        mock_exit.assert_called_once_with(1)
        worker._alert_admin_session_invalid.assert_awaited_once()
        worker.telegram_client.try_reconnect.assert_not_awaited()
