...
- `TELEGRAM_API_HASH`
- `TELEGRAM_BOT_TOKEN`
- `JAVA_API_KEY` — общий секрет с java-bot. Worker подставляет его в каждый
  запрос как header `X-API-Key`, иначе `ApiKeyFilter` вернёт 401 (исключение —
  `/api/health`).

На практике обычно нужны:
- `TELEGRAM_SESSION_STRING` (production)
- `REDIS_HOST`, `REDIS_PORT`, `REDIS_DB`, `REDIS_QUEUE_NAME`, `REDIS_PASSWORD` (опционально)
- `JAVA_API_BASE_URL` (по умолчанию `http://java-bot:8080`)
- `CACHE_ENABLED`, `CACHE_DB_PATH`, `CACHE_MAX_DISK_GB` (по умолчанию `25.0`)
- `JOB_TIMEOUT`, `MAX_RETRIES`, `RETRY_BASE_DELAY`, `RETRY_MAX_DELAY`
- `STATS_STREAM_KEY` (по умолчанию `stats:events`)
...
## Priority drain для subscription-задач

`QueueConsumer.get_job()` последовательно опрашивает три очереди через BLPOP с коротким таймаутом:
1. `telegram_export_express` (timeout 1 с)
2. `telegram_export` (timeout 2 с)
3. `telegram_export_subscription` (timeout 2 с)

Если все пусты — возвращает `None`, цикл продолжается. Так subscription-задачи не блокируют ручные экспорты: пользователи видят свои ручные задачи в приоритете.

### Обработка subscription iteration

Если `ExportRequest.source == "subscription"` и `messages_count == 0`:
- НЕ вызывается `send_response` (нет файла)
- `JavaBotClient.notify_subscription_empty(chat_id, chat_label, from_date, to_date)` отправляет текст в ЛС: «Итерация подписки по чату ... выполнена. Новых сообщений в окне ... → ... нет.»
- `mark_job_completed` вызывается как обычно.

Если `messages_count > 0` — стандартный путь с файлом.

Crash-safety: очередь `telegram_export_subscription_processing` staging восстанавливается при рестарте воркера (как express и main).
