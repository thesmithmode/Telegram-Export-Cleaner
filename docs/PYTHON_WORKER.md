...
Обычно нужны: `TELEGRAM_SESSION_STRING`, `REDIS_HOST/PORT/DB/PASSWORD`, `JAVA_API_BASE_URL` (default `http://java-bot:8080`), `CACHE_ENABLED/DB_PATH/MAX_DISK_GB` (default 25.0), `EXPORT_TEMP_DIR/EXPORT_MIN_FREE_DISK_MB`, `JOB_TIMEOUT/MAX_RETRIES`, `STATS_STREAM_KEY` (default `stats:events`).
