...
| `dashboard_users` | веб-логины; `bot_user_id` связывает USER с Telegram user_id; `language` хранит выбранный язык UI | UNIQUE `username` |
...
Язык UI хранится в `dashboard_users.language` и синхронизируется с языком бота
(`bot_users.language`). Смена через `POST /dashboard/api/me/settings/language`
обновляет обе таблицы. Поддерживаются 10 языков (см. `docs/I18N.md`).
