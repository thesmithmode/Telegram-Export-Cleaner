...
### Логика работы
- **Уникальность:** Не более одной активной ИЛИ приостановленной (PAUSED) подписки на пользователя (enforced partial unique index `uk_subscriptions_one_active_per_user` + проверка в сервисе). ARCHIVED подписки не ограничены.
- **Окно запуска:** Открывается за 30 минут до `desired_time_msk` и **остаётся открытым до конца суток** — планировщик делает catch-up, если воркер был занят. Повторный запуск в тех же сутках исключён через `isPeriodElapsed` (сравнение с `last_success_at + period - 30 мин`).
...
- **Формат дат в job:** Java усекает `Instant.now()` до секунд (`truncatedTo(ChronoUnit.SECONDS)`) перед форматированием `ISO_LOCAL_DATE_TIME` — Python worker валидирует строго до `HH:MM:SS`, наносекунды отправляют job в DLQ.
...
- Next run рассчитывается локально: `max(lastSuccessAt, lastRunAt, lastFailureAt) + period`. Если lastRunAt новее terminal-события ("Running…" — запуск начат, результат ещё не пришёл). Если ни одного события нет — "Waiting for first run".
