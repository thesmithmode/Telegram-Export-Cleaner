...
Реализован в `com/tcleaner/bot/ExportBot.java`. Бот принимает команды только в личных чатах и экспортирует только публичные группы, супергруппы и каналы с `@username`. Команды: `/start`, `/settings` (язык), `/cancel`.
...
**Числовые chat ID** в текущем UX напрямую не поддерживаются пользовательским вводом; worker дополнительно отклоняет приватные/непубличные резолвы без `username` перед чтением истории.
