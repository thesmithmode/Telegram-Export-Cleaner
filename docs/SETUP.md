...
- Это ожидаемо: экспорт разрешён только для публичных групп, супергрупп и каналов с `@username`. Приватные чаты без публичного username не экспортируются, даже если аккаунт worker-а является участником.
