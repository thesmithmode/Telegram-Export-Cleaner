...
Создайте A-запись:
```
tec.example.com → <IP сервера>
```
Проверка (DNS TTL может занять до 30 мин):
```bash
dig +short tec.example.com
```
...
TRAEFIK_DASHBOARD_DOMAIN=tec.example.com
```
...
Проверка:
```bash
curl -I https://tec.example.com/dashboard/login
# → HTTP/2 200
```
...
curl -sf https://tec.example.com/dashboard/login  # внешний
```
