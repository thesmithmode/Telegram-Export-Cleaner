...
с доменом `tec.searchingforgamesforever.online`.
...
Создайте A-запись:
```
tec.searchingforgamesforever.online → <IP сервера>
```
Проверка (DNS TTL может занять до 30 мин):
```bash
dig +short tec.searchingforgamesforever.online
```
...
TRAEFIK_DASHBOARD_DOMAIN=tec.searchingforgamesforever.online
```
...
Проверка:
```bash
curl -I https://tec.searchingforgamesforever.online/dashboard/login
# → HTTP/2 200
```
...
```bash
cd /root/telegram-export-cleaner
git pull
docker compose -f docker-compose.prod.yml --env-file .env pull
...
0 3 * * * docker exec telegram-export-redis redis-cli BGSAVE && \
    cp $(docker volume inspect telegram-cleaner_dashboard_data -f '{{.Mountpoint}}')/dashboard.db \
       /root/backups/dashboard-$(date +%F).db && \
    find /root/backups -name "dashboard-*.db" -mtime +30 -delete
```
...
# Логи java-bot (Flyway, Spring, ingestion)
...
curl -sf https://tec.searchingforgamesforever.online/dashboard/login  # внешний
```
...
| java-bot не стартует | `docker logs telegram-export-java-bot` — Flyway error? |
