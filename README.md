...
Пользователь может создать подписку на чат и получать автоматический экспорт с выбранной периодичностью (раз в день / 2 / 3 дня / неделю). Управление — через дашборд (`/dashboard/subscriptions`). Бот отправляет готовый файл в ЛС. Раз в 7 дней приходит запрос подтверждения с inline-кнопкой; если не подтвердить за 48 ч — подписка архивируется.
