...
- Экспортирует историю публичных групп, супергрупп и каналов по `@username` или `https://t.me/...` ссылке
...
2. Отправить `/start`
3. Отправить `@username` или `https://t.me/...` ссылку на публичную группу, супергруппу или канал
...
| `EXPORT_ARTIFACT_MAX_DISK_GB` | Лимит optional txt artifact-cache (`0` = `min(5GB, 20% CACHE_MAX_DISK_GB)`) |
...
## Подписки на периодические экспорты

Пользователь может создать подписку на публичный чат по `@username` или `https://t.me/...` и получать автоматический экспорт с выбранной периодичностью (раз в день / 2 / 3 дня / неделю). Управление — через дашборд (`/dashboard/subscriptions`). Бот отправляет готовый файл в ЛС. Раз в 7 дней приходит запрос подтверждения с inline-кнопкой; если не подтвердить за 48 ч — подписка архивируется.

Ограничения alpha-теста:
- Не более 1 активной подписки на пользователя
- Максимальный период — 7 дней
- Времена подписок — строго МСК (UTC+3)

См. подробности в [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#subscriptions) и [docs/DASHBOARD.md](docs/DASHBOARD.md#subscriptions).
